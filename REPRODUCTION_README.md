...
- 新增并执行测试，当前 50 条测试通过，覆盖本地 NTU-105 集成抽取、网页候选段落过滤、公式型 MOF 名称、`metal–organic` / `metal organic` 写法、无 `raw_record` 合并兼容、材料级去重、DOI 规范化、COF heuristic 抽取，以及 hydrazone、boronate ester、Knoevenagel、beta-ketoenamine 和 `reaction of A with B` 这类 COF 合成骨架，并额外校验 `℃` / mojibake 温度写法、常见溶剂/催化剂/碱缩写、气氛和基底字段。
