...
- 新增并执行测试，当前 68 条测试通过，覆盖本地 NTU-105 集成抽取、网页候选段落过滤、公式型 MOF 名称、`metal–organic` / `metal organic` 写法、无 `raw_record` 合并兼容、材料级去重、DOI 规范化、COF heuristic 抽取，以及 hydrazone、boronate ester、Knoevenagel、beta-ketoenamine 和 `reaction of A with B` 这类 COF 合成骨架，并额外校验 `℃` / mojibake 温度写法、常见溶剂/催化剂/碱缩写、气氛、界面、基底字段、真实开放文本中的通用 COF 描述、descriptor-before-family-name 和 vapor-induced conversion 热程序段落；benchmark 已纳入 TFPT-COF、TAPA-BTD-COF、W-A-X、PyTTA 系列 COF films、MIL-100 (Fe) 等真实开放文本，并增加 UTF-8/CJK mojibake 防回归检查。
