...
def main() -> None:
    parser = argparse.ArgumentParser(description="Extract normalized MOF records with legacy DigiMOF parsers.")
    ...
    records = iter_records(args.inputs, pages=pages, max_chars=max_chars)
    if not args.no_merge:
        records = merge_items(records)
    count = write_jsonl(records, args.output, mof_only=args.mof_only)
