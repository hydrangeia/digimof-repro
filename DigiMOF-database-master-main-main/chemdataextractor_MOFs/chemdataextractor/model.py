...
#Defines the topology (abrv) as a string
class Topology(BaseModel):
    abrv = StringType()

class BatteryCapacity(BaseModel):
    value = StringType(contextual=True)
    units = StringType(contextual=True)
    raw_value = StringType(contextual=True)
    raw_units = StringType(contextual=True)
    specifier = StringType(contextual=True)
    current_value = StringType(contextual=True)
    current_value1 = StringType(contextual=True)
    current_units = StringType(contextual=True)
    cycle_value = StringType(contextual=True)
    cycle_units = StringType(contextual=True)
...
class BatteryConductivity(BaseModel):
    value = StringType(contextual=True)
    units = StringType(contextual=True)
    raw_value = StringType(contextual=True)
    raw_units = StringType(contextual=True)
...
class BatteryCoulombic(BaseModel):
    value = StringType(contextual=True)
    units = StringType(contextual=True)
    raw_value = StringType(contextual=True)
    raw_units = StringType(contextual=True)
...
#Defines the linker and metal precursor objects as strings
class LinkerRoute(BaseModel):
    linker = StringType()
    metal_precursor = StringType()
    linker_quantifier= StringType()
    metal_quantifier = StringType()
...
class Compound(BaseModel):
    ...
    capacities = ListType(ModelType(BatteryCapacity))
    voltages = ListType(ModelType(BatteryVoltage))
    conductivities = ListType(ModelType(BatteryConductivity))
    topologies = ListType(ModelType(Topology))
    coulombics = ListType(ModelType(BatteryCoulombic))
    energies = ListType(ModelType(BatteryEnergy))
    adsorption_enthalpies = ListType(ModelType(AdsorptionEnthalpy))
    synthesis_routes = ListType(ModelType(SynthesisRoute))
    linker_routes = ListType(ModelType(LinkerRoute))
