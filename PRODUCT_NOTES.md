...
MOF name extraction now also preserves formula-style framework identifiers from real open-access summary paragraphs such as `The solvothermal reaction ... gave rise to a metal-organic framework (MOF), {(H 3 O + ) 2 [Ca(NDC)(C 2 H 5 O)(OH)]} 4 ·1.1H 2 O`, so route-first literature wording no longer drops the framework name when the descriptor introduces the identifier after the synthesis clause.
COF monomer extraction now also captures real on-surface `by co-depositing A and B on Au(111)` wording, so open-access silabenzene COF abstracts retain reagent-pair evidence even when the synthesis paragraph never uses the usual `from A and B` or `coupling of A with B` phrasing.
COF route extraction now also normalizes real on-surface `Ullmann-like ... reaction` wording to the canonical `Ullmann coupling` route value, and COF time extraction no longer misreads dimensional labels such as `2D COFs` as synthesis durations.
...
MOF name extraction now also preserves common framework identifiers such as `ZIF-8` when a real open-access synthesis abstract uses the shorthand name next to synthesis wording like `constructed under ambient conditions` instead of repeating the full `metal-organic framework (MOF)` descriptor.
...
MOF name extraction now also preserves real fabricated-monolayer wording such as `successfully fabricated a 2D monolayer Cu-dicyanoanthracene (DCA) MOF`, so open-access surface-synthesis abstracts keep the complete framework identifier instead of fragmented chemical/entity names.
COF atmosphere extraction now also preserves real on-surface synthesis condition wording such as `ultrahigh vacuum` and `backfilled hydrogen gas`, so Ullmann COF abstracts retain the scoped vacuum/reducing-gas evidence without inferring unrelated characterization atmospheres.
COF monomer extraction now ignores standalone narrative `monomer`/`monomers` values, so real open-access Ullmann endpoint wording such as `from the monomer` does not create an unsupported monomer field while named phrases like `porphyrin monomer` remain extractable.
